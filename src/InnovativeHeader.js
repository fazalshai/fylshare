    ...
    return (
        <>
            ...
            <motion.header
                ...
                className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 hidden md:flex items-center gap-1 px-3 py-2 rounded-full border border-white/10 bg-black/30 backdrop-blur-md shadow-[0_8px_32px_0_rgba(31,38,135,0.37)]"
            >
                {/* Logo Area */}
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link key={item.name} to={item.path} className="relative">
                                <motion.div
                                    className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-colors ${isActive ? "text-white" : "text-gray-400 hover:text-white hover:bg-white/10"
                                        }`}
